...
//uint8_t allPixels[WIDTH * HEIGHT * 4];

int main()
{
	sf::String window_title = "Mandelbrot";
	...
	{
		farm.add_task(new computemdb(left, right, top, bottom, i, (i + 1), i));
	}
	...
	uint8_t* allPixels = (uint8_t*)image;
	texture.update(allPixels);
		...
		while (window.pollEvent(event))
		{
			...
			if (sf::Mouse::isButtonPressed(sf::Mouse::Left))
			{
				...
				for (int i = 0; i < 10; ++i)
				{
					...
					farm.add_task(new computemdb(-0.751085, -0.734975, 0.118378, 0.134488, i, (i + 1), i));
				}
